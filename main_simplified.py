...
from cifar100.maskcl2 import *
# from ptflops import get_model_complexity_info
#from cifar100.thop import profile, clever_format
from cifar100.manipulate import SubDataset
torch.backends.cudnn.benchmark = True
_logger = logging.getLogger('train')
from cifar100.available import AVAILABLE_DATASETS, AVAILABLE_TRANSFORMS, DATASET_CONFIGS
from torch.utils.data.dataloader import DataLoader
from torch.utils.data import ConcatDataset
import copy
from cifar100.vgg_snn import SNN
...
parser.add_argument('--task_num', type=int, default=5, metavar='N',
...
def main():
    ...
    output_dir = '/lustre/groups/adamgrp/ContLearning/Brain-Cog/examples/Structural_Development/DSD-SNN/output/'
    #output_base = args.output if args.output else './output'
    output_base = '/lustre/groups/adamgrp/ContLearning/Brain-Cog/examples/Structural_Development/DSD-SNN/output'
    exp_name = '-'.join([
    ...
    print("checking output dir", output_base)
    ...
    batch_size=args.batch_size
    data_dir = '/lustre/groups/adamgrp/ContLearning/Brain-Cog/examples/Structural_Development/DSD-SNN/data0/datasets/'
    ...
    loader_his=[]
    task_ready={}
    task_accuracies = {task: [] for task in range(args.task_num)}
    ...
    try:  # train the model
        ...
        for task in range(len(train_datasets)):
            ...
            for epoch in range(epochs):
                ...
                if task == 0:
                    ...
                    train_epoch(epoch, task, model, loader_train, optimizer, train_loss_fn, args, mat, task_ready, taskww=taskww,
                            lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir, regularization_terms=regularization_terms)
            print("==============================================================================================================")
            print(f"Accuracies for Task {task}: {task_accuracies[task]}")
            ...
            for t in ta_his:
                ...
                accuracy = validate(t, model, loader_his, validate_loss_fn, args, mat)
                task_accuracies[t].append(accuracy)  # Store accuracy for this task and epoch
...
#     _logger.info('*** Best metric: {0} (epoch {1})'.format(best_metric, best_epoch))
    ...
    for task, accuracies in task_accuracies.items():
        print("task accuracies", task_accuracies[task])
        plt.plot(accuracies, label=f'Task {task}')
    plt.title('Task Accuracies over Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()
    plt.savefig(os.path.join(output_dir, '/lustre/groups/adamgrp/ContLearning/Brain-Cog/task_accuracies.png'))
...
def validate(task, model, loader, loss_fn, args, mat, log_suffix='', visualize=False, spike_rate=False):
    ...
    print("testtttttttttt", top1_m.avg)
    return top1_m.avg  # Return the average top-1 accuracy
